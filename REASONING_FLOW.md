...
- 助手消息区域改为「模型标识胶囊 → 思维链组件 → 回复文本」的纵向布局，胶囊内容为“模型分组名（昵称） | 模型名”；
- 当最终回复尚未开始输出时，以 `StreamingResponsePlaceholder` 胶囊动画占位：使用正弦波驱动的亮色高光在胶囊内部左右滑动，直至真实文本流式输出；
- 非推理模型同样复用该占位动画，让“等待回复”的状态具备统一的动态反馈。
